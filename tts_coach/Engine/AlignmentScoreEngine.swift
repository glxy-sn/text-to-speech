...
struct PhoneScore: Identifiable {
    let id = UUID()
    let symbol: String
    let gopScore: Float   // 0.0 (incorrect) → 1.0 (perfect)
    let frames: Int
}
...
class AlignmentScorerEngine {
    ...
    func scorePronunciation(
        userLogits: MLMultiArray,
        targetPhonemes: [String],
        vocabulary: [String],
        greedyAnchors: [(symbol: String, frame: Int)]
    ) -> [PhoneScore] {
        ...
        let probs = MathService.softmaxVectorized(
            logits: ptr, frameCount: totalFrames, vocabSize: vocabSize
        )

        return runCTCForwardBackward(
            probs: probs, targets: targetPhonemes, vocab: vocabulary,
            totalFrames: totalFrames, vocabSize: vocabSize
        )
    }

    private func runCTCForwardBackward(
        probs: [Float], targets: [String], vocab: [String],
        totalFrames: Int, vocabSize: Int
    ) -> [PhoneScore] {
        ...
        for k in 0..<M {
            // Strip length-mark (ː) so the symbol matches the vocab entry
            let symbol = targets[k].replacingOccurrences(of: "ː", with: "")
            E[2 * k + 1] = vocab.firstIndex(of: symbol) ?? blankIndex
        }

        // MARK: Forward pass (alpha)
        var alpha = [[Float]](
            repeating: [Float](repeating: -Float.greatestFiniteMagnitude, count: L),
            count: totalFrames
        )
        ...
        for t in 1..<totalFrames {
            ...
            for s in 0..<L {
                ...
                if s > 1 && E[s] != blankIndex && E[s] != E[s - 2] {
                    a3 = alpha[t - 1][s - 2]
                }
